...
class Processor:
    ...
    Attributes:
        cat_cols (list[str] | None): List of categorical column names.
        num_cols (list[str] | None): List of numerical column names.
    ...
    def _get_cat_cols(self, data: pd.DataFrame, target: str):
    ...
    def _get_num_cols(self, data: pd.DataFrame, target: str):
    ...
    def run(
        ...
        self._get_cat_cols(data=train_data, target=target)
        if self.cat_cols:
            logger.info(
                f"Categorical columns detected: {', '.join(self.cat_cols)}"
            )

        self._get_num_cols(data=train_data, target=target)
        if self.num_cols:
            logger.info(
